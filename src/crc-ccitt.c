...
/**
 * crc_ccitt - recompute the CRC (CRC-CCITT variant) for the data buffer.
...
{
	while (len--)
		crc = crc_ccitt_false_byte(crc, *buf++);
	return crc;
}

void __attribute__((constructor)) crc_ccitt_selftest(void)
{
	uint16_t res = crc_ccitt_false_byte(0xffff, 0xc0);
	assert(res == 0x38bc);
}
